...
那么如何对$\mathbb{V}_{W}(\hat{\tau}^{\mathrm{dif}})$进行估计呢？从上面的式子中不难看出，$S_{t}^{2}$以及$S_{c}^{2}$我们可以分别用从处理组和控制组观测结果的样本方差进行估计，但$S_{t c}^{2}$中同时包含同一个体在两种处理水平下的潜在结果，无法对其直接进行计算。但注意到，当个体水平的因果效应$Y_{i}(1)-Y_{i}(0) \equiv \tau$时，$S_{t c}^{2}=0$，此时，$\mathbb{V}_{W}(\hat{\tau}^{\mathrm{dif}})=\frac{S_{c}^{2}}{N_{\mathrm{c}}}+\frac{S_{t}^{2}}{N_{\mathrm{t}}}$，此时我们可以给出它的一个无偏估计：
