...
其中$\bar{Y}_{\mathrm{t}}^{\mathrm{obs}}=\sum_{i: W_{i}=1} Y_{i}^{\mathrm{obs}} / N_{\mathrm{t}}$，$\bar{Y}_{\mathrm{c}}^{\mathrm{obs}}=\sum_{i: W_{i}=0} Y_{i}^{\mathrm{obs}} / N_{\mathrm{c}}$，$N_{\mathrm{c}}=\sum_{i=1}^{N}\left(1-W_{i}\right)$，$N_{\mathrm{t}}=\sum_{i=1}^{N}\left(W_{i}\right)$。
...
从上表中我们可以看出，20种分配形式种，有16种形式下$T$的取值大于等于$T^{obs}=1$，从而该检验问题对应的$p$值为$16/20 = 0. 80$。根据$T$的随机化分布，我们也可以给出相应的区间估计。

在这个例子中，选取的检验统计量为两组潜在结果的均值差异，在实际问题中可根据具体问题选择不同的检验统计量，如可选择分位数、两样本均值检验的T-统计量、秩统计量等等，也可对潜在结果进行对数变换等得到不同形式的检验统计量。此外，由于这个例子中的总体个数较少，随机化的所有分配形式的可能性也较少，计算$T$的随机化分布相对容易，但在实际问题中，总体数量通常较多，此时要穷举出所有分配形式下的$T$的取值计算消耗较大，这是我们可以通过随机选取一部分分配方案计算出一个近似的$p$值，当随机模拟次数重复足够多次时，得到的$p$值精度也可以得到保证。
